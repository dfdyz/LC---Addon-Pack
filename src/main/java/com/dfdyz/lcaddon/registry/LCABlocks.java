...
public class LCABlocks {
    ...
    private static <T extends Block> RegistryObject<Item> registerBlockItem(String name, RegistryObject<T> block, int max_stack) {
        RegistryObject<Item> registry = LCAItems.ITEMS.register(name, () -> {
            return new BlockItem(block.get(), (new Item.Properties()).stacksTo(max_stack).tab(AllTabs.MIDI_TAB));
        });
    ...
    static {
        ...
        ELECTRONIC_PIANO = registerBlock("roland_fp30x", () -> {
            return new ElectronicPianoBlock(BlockBehaviour.Properties.of(Material.WOOD)
                    .strength(1.0F)
            );
        });

        STEINWAY_D_PIANO = registerBlock("steinway_d", () -> {
            return new SteinwayDPiano(BlockBehaviour.Properties.of(Material.WOOD)
                    .strength(1.0F).noOcclusion()
            );
        });

        EPIANO_CHAIR = registerBlock("epiano_chair_block", () -> {
            return new EPianoChairBlock(BlockBehaviour.Properties.of(Material.WOOD)
                    .strength(1.0F)
            );
        });

        PIANO_CHAIR = registerBlock("piano_chair_block", () -> {
            return new PianoChairBlock(BlockBehaviour.Properties.of(Material.WOOD)
                    .strength(1.0F)
            );
        });
    }
