...
public class PatchedPianoScreen extends ClavichordScreen {
    ...
    protected void init() {
        ...
        this.LinkLamp = new CommonButton(this.width / 2 - 190, this.height / 2 + 60, 180, 20,
                new TextComponent((linkedLamp != null && !linkedLamp.isRemoved()) ? linkedLamp.position().toString() :  "LinkLamp" ),
                (button) ->
                {
                    linkedLamp = EntityManager.GetNearestInRange(minecraft.level, pos, 3f, MoonLampEntity.class);
                    LinkLamp.setMessage(new TextComponent((linkedLamp != null && !linkedLamp.isRemoved()) ? linkedLamp.position().toString() :  "LinkLamp" ));
                });
